...
public:
    ...
    ClientConnectionHandler(const std::string &ip, int port) {
        ...
        std::cout << "Connecting to Server on port " << port << "...\n";
    ...
    void update() {
        ...
        if (int activity = select(m_socket + 1, &read_set, nullptr, nullptr, nullptr); activity < 0) {
    ...
    void sendCommand(const std::unique_ptr<Command> &cmd) {
